...
(function(){
  ...
  function bloque(h){
    return '<div class="dg-item '+h.tipo+'">'+
      ...
      '</div>'+
      (h.causas ? '<p class="dg-causas"><b>Sugerencia:</b> '+h.causas+'</p>' : '')+
  ...
  function pintar(){
    ...
    return '<div class="dg-wrap">'+
      ...
      '<p class="dg-sub">Lo que Vantik alcanza a ver en sus '+total+' animales. '+
        'Con sugerencias de que se puede hacer. La ultima palabra es suya. '+
      ...
      (obs.length ?
        ...
        '<p class="dg-secsub">Comparado con las referencias que usted fijo. Si su manejo es otro, '+
        'cambie la referencia o silencie el aviso.</p>'+
        obs.map(bloque).join('') : '')+
      ...
      '<p class="dg-nota">Las sugerencias son generales. Usted conoce su finca, su clima y sus costos:<br>'+
        'tomelas como un punto de partida, no como una regla.</p>'+
    '</div>';
  }
  ...
  /* boton en Inicio */
  /* --- se anuncia en el registro: Inicio y el tutorial lo recogen solos --- */
  function registrarse(){
    if(!window.Vantik) return;
    Vantik.registrar({
      id:'revision', nombre:'Revision', icono:'&#9873;', tono:'verde', orden:10,
      accion: function(){ abrirDiagnostico(); },
      avisos: function(){
        if(!Vantik.u.hayDatos()) return [];
        var res = analizar();
        return res.H.filter(function(h){ return h.tipo === 'hecho'; }).map(function(h){
          return {nivel:'alta', n:h.animales.length, titulo:h.titulo, detalle:h.texto,
                  accion:function(){ abrirDiagnostico(); }};
        });
      },
      pasos: function(){
        return [{titulo:'Revisa el estado del hato',
                 texto:'Vantik mira tus datos y te muestra lo que puede estar pendiente.',
                 hecho: Vantik.u.hayDatos()}];
      }
    });
    if(typeof abrirCargaRapida === 'function'){
      Vantik.registrar({id:'carga-rapida', nombre:'Carga rapida', icono:'&#43;', tono:'verde', orden:20,
        accion:function(){ abrirCargaRapida(); },
        pasos:function(){ return [{titulo:'Carga tus animales',
          texto:'Con la carga rapida metes varios seguidos sin repetir los mismos datos.',
          hecho: Vantik.u.animales().length > 0}]; }});
    }
  }
